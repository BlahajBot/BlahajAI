...
class HermesCLI:
    ...
    def __init__(
        ...
        # Ephemeral system prompt: env var takes precedence, then selected
        # config personality, then explicit agent.system_prompt.
        self.personalities = CLI_CONFIG["agent"].get("personalities", {})
        env_system_prompt = os.getenv("HERMES_EPHEMERAL_SYSTEM_PROMPT", "")
        selected_personality = str(
            CLI_CONFIG.get("display", {}).get("personality", "") or ""
        ).strip().lower()
        if env_system_prompt:
            self.system_prompt = env_system_prompt
        elif selected_personality and selected_personality not in {"none", "default", "neutral"} and selected_personality in self.personalities:
            self.system_prompt = self._wrap_personality_prompt(
                selected_personality,
                self._resolve_personality_prompt(self.personalities[selected_personality]),
            )
        else:
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        if len(parts) > 1:
            ...
            if personality_name in {"none", "default", "neutral"}:
                ...
                saved = save_config_value("display.personality", "none") and saved
                if saved:
                ...
                saved = save_config_value("display.personality", personality_name) and saved
                if saved:
