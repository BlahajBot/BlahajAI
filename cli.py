...
from utils import base_url_host_matches, is_truthy_value, fast_safe_load
    ...
    def _resolve_turn_agent_config(self, user_message: str, *, spark_lane: bool = False) -> dict:
        ...
        if spark_lane:
            ...
            if runtime.get("provider") != _SPARK_PROVIDER:
                ...
                resolved = resolve_runtime_provider(requested=_SPARK_PROVIDER)
                runtime.update(
                    {
            ...
            while not self._should_exit:
                try:
                    ...
                    if not _file_drop and isinstance(user_input, str) and _looks_like_slash_command(user_input):
                        _cprint(f"\n⚙️  {user_input}")
                        _canonical = _canonical_command_name(user_input)
                        if _canonical == "spark":
                            payload = user_input.split(maxsplit=1)[1].strip() if len(user_input.split(maxsplit=1)) > 1 else ""
                            if not payload:
