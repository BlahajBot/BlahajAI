...
def _setup_tts_provider(config: dict):
    ...
    choices.extend(
        [
            ...
            "Groq TTS (fast, free tier, needs API key)",
    ...
    providers.extend(["edge", "elevenlabs", "openai", "groq", "xai", "minimax", "mistral", "gemini", "neutts", "kittentts"])
    ...
    if selected == "neutts":
        ...
        if not existing:
            print()
            api_key = prompt("Groq API key for TTS", password=True)
            if api_key:
                save_env_value("GROQ_API_KEY", api_key)
                print_success("Groq TTS API key saved")
            else:
                print_warning("No API key provided. Falling back to Edge TTS.")
                selected = "edge"
