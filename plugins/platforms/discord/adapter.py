...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        if os.getenv("DISCORD_REGISTER_SKILL_COMMANDS", "true").strip().lower() not in {
            "0", "false", "no", "off",
        }:
        ...
        if not is_thread and not isinstance(message.channel, discord.DMChannel):
            ...
            if auto_thread and not skip_thread and not is_voice_linked_channel and not is_reply_message:
                ...
                if thread:
                    ...
                    self._threads.mark(thread_id)
                    # Discord emits a second MESSAGE_CREATE for the thread
                    # starter with id == thread.id; pre-seed dedup so it cannot
                    # trigger a duplicate agent run.
...
def _component_check_auth(
    ...
    Component button clicks must be explicitly authorized by a Discord
    user/role allowlist, a global user allowlist, an explicit allow-all flag,
    or the pairing store (``hermes pairing approve``).
    """
