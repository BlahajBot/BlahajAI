...
class _AnthropicCompletionsAdapter:
    ...
    @staticmethod
    def _contains_image_parts(messages: Any) -> bool:
        ...
        for msg in messages:
            ...
            for part in content:
                if not isinstance(part, dict):
                    continue
                if str(part.get("type", "")).lower() in {"image_url", "input_image", "image"}:
                    return True
        return False

    @staticmethod
    def _redact_anthropic_kwargs_for_log(payload: Dict[str, Any]) -> Dict[str, Any]:
        """Return a log-safe copy of anthropic kwargs for multimodal debugging."""
        ...
        for msg in redacted.get("messages", []):
            ...
            for block in content:
                ...
                if src_type == "base64":
                    ...
                    source["data"] = f"<redacted:{len(data)} chars>"
                elif src_type == "url":
                    url = str(source.get("url", "") or "")
                    source["url"] = url[:96] + ("..." if len(url) > 96 else "")
    ...
    def create(self, **kwargs) -> Any:
        ...
        try:
            ...
            if self._contains_image_parts(messages):
                logger.error(
                    "Anthropic vision request failed: model=%s oauth=%s payload=%s",
                    model,
                    self._is_oauth,
                    json.dumps(
                        self._redact_anthropic_kwargs_for_log(anthropic_kwargs),
                        ensure_ascii=False,
                    ),
                    exc_info=True,
                )
