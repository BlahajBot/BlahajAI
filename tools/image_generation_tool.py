...
IMAGE_GENERATE_SCHEMA = {
    ...
    "description": (
        ...
        "edit / transform / restyle an attached image when the active model "
        "supports it. Pass `input_image` to edit that image; omit it for "
        "text-to-image. The underlying backend (FAL, OpenAI, xAI, etc.) "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "quality": {
                ...
                "description": (
                    "Optional quality tier for providers that support per-call "
                    "quality selection. Unsupported providers ignore it and use "
            ...
            "input_image": {
                "type": "string",
                "description": (
                    "Optional attached image to edit, transform, or restyle. "
                    "Use an absolute local file path, HTTP(S) URL, or "
                    "data:image/... URL from the conversation. Only honored by "
                    "models that support image editing; omit it for normal "
...
def _handle_image_generate(args, **kw):
    ...
    image_url = args.get("input_image") or args.get("image_url")
...
def _build_dynamic_image_schema() -> Dict[str, Any]:
    ...
    if "image" in modalities and "text" in modalities:
        ...
        ref_note = (
            f"; supports up to {max_refs} reference image(s) internally"
            if max_refs and max_refs > 1
            else ""
        )
        parts.append(
            "- supports both text-to-image (omit input_image) and "
            f"image-to-image / editing (pass input_image){ref_note} — "
            "routes automatically"
        )
    elif "image" in modalities and "text" not in modalities:
        parts.append(
            "- this model is image-to-image / edit only — input_image is REQUIRED"
        )
    else:
        parts.append(
            "- this model is text-to-image only — it is NOT capable of "
            "image-to-image / editing; do not pass input_image "
            "(it will be rejected). Provide a "
            "text-only prompt."
        )
